...
ASSET_VALUE  = 3_500_000_00
TOTAL_TOKENS = 1_000_000 * 10**18
INITIAL_MINT = 100_000  * 10**18
...
def main():
    ...
    deploy_tx = factory.constructor(
        TOKEN_NAME, TOKEN_SYMBOL, ASSET_NAME, ASSET_DESC, ASSET_LOC, ASSET_VALUE, TOTAL_TOKENS,
    ).build_transaction({
    ...
    mint_tx = token.functions.mint(account.address, INITIAL_MINT).build_transaction({
        "from":     account.address,
        "nonce":    w3.eth.get_transaction_count(account.address),
        "gas":      200_000,
        "gasPrice": w3.eth.gas_price,
