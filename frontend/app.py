"""
app.py — Streamlit frontend for the RWA Tokenization Platform.

Run with:  streamlit run frontend/app.py

Streamlit is a Python library that turns a Python script into a web app.
Every time the user clicks a button or changes an input, Streamlit
re-runs the entire script from top to bottom and re-renders the page.
This is very different from React — there is no component tree, just a script.

How this app talks to the contract:
  - READING data (balances, asset info) → free, no wallet needed
    Uses: contract.functions.someViewFn().call()
  - WRITING data (mint, transfer, whitelist) → costs gas, needs a private key
    Uses: build_transaction() → sign_transaction() → send_raw_transaction()
    The private key NEVER leaves your machine — you enter it in the sidebar.

⚠️  IMPORTANT: Only use test account private keys here (Hardhat/Anvil accounts).
    Never enter a real wallet private key into any web app.
"""
...
def load_contract_data():
    """Load ABI and deployed address from files generated by compile + deploy scripts."""
...
def get_contract(w3, abi, address):
    """Create a web3.py Contract object. This is the Python handle to the on-chain contract."""
    return w3.eth.contract(address=address, abi=abi)


def send_tx(w3, contract_fn, private_key, gas=300_000):
    """
    Sign and send a write transaction.

    Why we do it this way (build → sign → send) instead of just calling transact():
      - build_transaction() creates the unsigned tx dict
      - sign_transaction() signs it locally with your private key (never sent to the node)
      - send_raw_transaction() sends the signed bytes — the node can't see your key
      This is the safest pattern for non-custodial wallets.
    """
...
def fmt_tokens(raw_wei):
    """Convert raw token units (wei-scale) to a human-readable number."""
    if raw_wei is None:
        return "—"
...
with st.sidebar:
    ...
    rpc_url = st.text_input(
        ...
        help="Your local Hardhat node or any Ethereum JSON-RPC endpoint",
    )
    ...
    private_key = st.text_input(
        ...
        help="Hardhat test account #0 private key (only use test keys here!)",
    )
...
# ── Read on-chain data ────────────────────────────────────────────────────────
# All these are free view calls — no gas, no signing needed.
...
# Detect if connected wallet is the owner (for admin tab gating)
user_address = None
is_owner     = False
if private_key:
    ...
    if asset_btn:
        with st.spinner("Updating…"):
            try:
                send_tx(
                    w3,
                    contract.functions.updateAssetDetails(
                        new_name, new_desc, int(new_value * 100)
                    ),
                    private_key,
                )
